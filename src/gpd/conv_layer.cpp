...
ConvLayer::ConvLayer(int width, int height, int depth, int num_filters, int spatial_extent, int stride, int padding)
  : w1(width), h1(height), d1(depth), d2(num_filters), f(spatial_extent), s(stride)
{
...
Eigen::MatrixXf ConvLayer::forward(const std::vector<float>& x) const
{
  ...
  Eigen::MatrixXf X_col = ConvLayer::RowMajorMatrixMap(x_col_vec.data(), X_col_r, X_col_c);

  // Convert weights vector to matrix where each row is a kernel.
  Eigen::MatrixXf W_row = ConvLayer::RowMajorMatrixMap(weights_.data(), W_row_r, W_row_c);

  // Convert biases array to Eigen vector.
  Eigen::VectorXf b = Eigen::Map<const Eigen::VectorXf>(biases_.data(), biases_.size());

  // Calculate the convolution by calculating the dot product of W_row and X_col.
  Eigen::MatrixXf H = forward(W_row, b, X_col);
  return H;
}


Eigen::MatrixXf ConvLayer::forward(const Eigen::MatrixXf& W, const Eigen::VectorXf& b, const Eigen::MatrixXf& X) const
{
...
void ConvLayer::imageToColumns(const float* data_im, const int channels, const int height, const int width,
